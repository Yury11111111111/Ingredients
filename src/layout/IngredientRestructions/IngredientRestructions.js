...
export default function IngredientRestructions() {
  const [ingredients, setIngredients] = useState([
    "Грибы",
    ["Грибы приморские", "Грибы Отморские", "Грибы заморские"],
    "Овощи",
    ["Помидор", "Огурец", "Кабачок"],
    "Мясо",
    ["Жирное", "Хрупкое", "Заморское"],
    "Оливки",
    [
      "Оливки японские",
      "Оливки Отморские",
        ...
        setCheckedCategories((prev) => ({
          ...
          [ing]: {
            ...
            subCategorys: Array.isArray(nextIngredient)
              ? Object.fromEntries(
                  nextIngredient.map((next) => [next, { checked: false }])
                )
              : {},
          },
        }));
  ...
  const toggleCategoryCheck = (category) => {
    setCheckedCategories((prev) => ({
      ...
      [category]: {
        ...
        subCategorys: checkedCategories[category].subCategorys
          ? Object.fromEntries(
              Object.entries(prev[category].subCategorys).map(
                ([subCategory, subData]) => [
                  subCategory,
  ...
  const toggleSubCategory = (category, subCategory) => {
    setCheckedCategories((prev) => {
      ...
      const allChecked = Object.values(updatedSubCategories).every(
        (sub) => sub.checked
      );
  ...
  const filterIngredients = () => {
    return ingredients.reduce((acc, item, index) => {
      if (typeof item === "string") {
        ...
        const subCategories = Array.isArray(ingredients[nextIndex])
  ...
  const saveSelectedIngredients = () => {
    // 1. Собираем выбранные ингредиенты
    const selected = [];
    for (const category in checkedCategories) {
      if (checkedCategories[category].checked) selected.push(category);
      if (checkedCategories[category].subCategorys) {
        for (const subCategory in checkedCategories[category].subCategorys) {
          if (checkedCategories[category].subCategorys[subCategory].checked) {
    ...
    // 2. Получаем ration-name из localStorage/состояния или запрашиваем у пользователя
    const rationName = localStorage.getItem('ration-name') || prompt("Введите название рациона:");
  ...
  return (
    <div className="ing-restriction">
                  ...
                  return (
                    <div key={index} className="ing-restriction__category">
                      <div className="ing-restriction__category-header">
                        ...
                        {Array.isArray(
                          ingredients[ingredients.indexOf(item) + 1]
                        ) ? (
                          <img
                            src={Arrow}
                            alt="arrow"
                      ...
                      {expandedCategories[item] &&
                        Array.isArray(
                          ...
                          <div className="ing-restriction__subcategories">
                            {ingredients[ingredients.indexOf(item) + 1].map(
                              (subItem, subIndex) => (
                                <div
                                  key={subIndex}
                                ...
                                >
                                  <input
                                    ...
                                    checked={
                                      checkedCategories[item]?.subCategorys[
                                        subItem
                                      ]?.checked || false
                                    }
                                    onChange={() =>
                                      toggleSubCategory(item, subItem)
                                    }
                                  />
      ...
      <Button type="submit" text="Сохранить" onClick={saveSelectedIngredients} />
    </div>
  );
}
